...
export default async function CoursesPage() {
  ...
  const familyName =
    session?.user && typeof (session.user as any).family_name === 'string'
      ...
      : ''
  const requireProfileCompletion = isAuthenticated && (!givenName || !familyName)

  const profileCompletionParams = new URLSearchParams()
  profileCompletionParams.set('pp', '1')
  profileCompletionParams.set('returnTo', '/courses')
  if (orgBranding?.id) profileCompletionParams.set('organization', orgBranding.id)
  const profileCompletionUrl = `/api/auth/login?${profileCompletionParams.toString()}`
