...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Fetch current user_metadata so we can merge enrolled courses safely.
    ...
    const enrolled = Array.from(new Set([...existing, courseId]))
    ...
    if (!patchResp.ok) {
      ...
      return NextResponse.json(
        { error: 'Failed to enroll in course', details: errorText },
